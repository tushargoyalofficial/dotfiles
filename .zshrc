...
#
# ~/.zshrc is for INTERACTIVE ZSH configuration only.
#
# Environment variables, PATH and toolchain initialization that GUI
# applications may need belong in ~/.zprofile.
# ============================================================================
...
# fzf-tab for beautiful tab completion
...
zstyle ':fzf-tab:complete:cd:*' \
    fzf-preview \
    'eza -1a --color=always $realpath 2>/dev/null || ls -la --color=always $realpath'
...
zstyle ':fzf-tab:complete:man:*' \
    fzf-preview \
    'man $word | bat -plman --color=always 2>/dev/null || man $word'
...
if command -v fzf >/dev/null 2>&1; then
    ...
    if command -v fd >/dev/null 2>&1; then
        ...
        export FZF_ALT_C_COMMAND='find . -type d --hidden --follow --exclude .git 2>/dev/null'
...
if command -v oh-my-posh >/dev/null 2>&1; then
    ...
    if command -v brew >/dev/null 2>&1 && \
       [[ -f "$(brew --prefix oh-my-posh)/themes/pure.omp.json" ]]; then
...
# ============================================================================
# LANGUAGE MANAGERS - INTERACTIVE SHELL INTEGRATIONS
# ============================================================================

# NOTE:
#
# mise is intentionally NOT initialized here.
#
# mise belongs in ~/.zprofile so its shims are available to GUI applications
# and IDEs as part of the login environment.
#
# Only shell-specific integrations belong here.
...
sysinfo() {
    ...
    echo "OS: $(lsb_release -d 2>/dev/null | \
        cut -d':' -f2 | xargs || \
        cat /etc/os-release | \
        grep PRETTY_NAME | \
        cut -d'=' -f2 | \
...
# Machine-specific/user-specific interactive configuration.
#
# Keep ~/.zshrc.local optional so the main configuration stays portable.
